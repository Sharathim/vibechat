...
@chat_bp.route('/conversations/with/<int:other_user_id>', methods=['POST'])
...
def clear_chat(conv_id):
    ...
    if err:
        return err, code
    execute_db(
        "DELETE FROM messages WHERE conversation_id = %s AND sender_id = %s",
        (conv_id, user['id'])
    )
...
@chat_bp.route('/conversations/<int:conv_id>/block-vibe', methods=['POST'])
def block_vibe(conv_id):
    ...
    execute_db(
        """INSERT INTO blocked_vibe_users (user_id, blocked_user_id)
